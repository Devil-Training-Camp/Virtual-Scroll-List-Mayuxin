...
const setStartOffset = () => {
  if (startIndex.value > 0) {
    let size = positionList.value[startIndex.value].top - positionList.value[startIndex.value - aboveCount.value].top
...
<template>
  <div ref="containerRef" class="infinite-list-container" @scroll="onScroll">
    ...
    <div class="infinite-list" :style="{ transform: getTransform }">
      <div v-for="item in visibleData" :key="item._id" :tId="item._id" class="infinite-list-item" ref="itemsRef">
        <slot :item="item"></slot>
      </div>
    </div>
  </div>
